...
const [update, setUpdate]=useState(false)
  return (
    ...
    />}
    {(!isUpdatable|| update)&&  <div className='grid grid-cols-1  gap-x-8 gap-y-3 lg:grid-cols-2'>
        ...
        <Select styles={multiSelectStyle}   label={TextConstant.municipality} {...form.getInputProps('municipalityId')}  key={form.key('municipalityId')} data={ arrondissment.data?.data.map(e=>({label:e.name!, value:`${e.id}`})) }/>
          <Select styles={multiSelectStyle}   label={TextConstant.district}  {...form.getInputProps('cityId')}  key={form.key('cityId')} data={ commune.data?.data.map(e=>({label:e.name!, value:`${e.id}`})) }/>
      </div>}
      </>
  )
}
