...
export const baseApi = createApi({
  ...
  endpoints: (builder) => ({
    ...
    getEts: builder.query<BaseResponse<EstablishmentTypeDto[]>, void>({
      query: () => `/establishment_type/all`,
    }),
    getEtsCompany: builder.query<BaseResponse<EstablishmentTypeDto[]>, string>({
      query: () => `/ws/company/establishment_type/all`,
    }),
    getEtsCompanyById: builder.query<
      BaseResponse<EstablishmentTypeDto>,
      string
    >({
      query: (id) => `/ws/company/establishment_type/by_id/${id}`,
    }),
    getCompanyDetails: builder.query<BaseResponse<CompanyDto>, string>({
      query: (id) => `/ws/company/details/${id}`,
