...
import { Avatar, Text,Container, Group, Title } from '@mantine/core';
...
export const CompanyDetails = () => {
  ...
  return (
    <Fetchingdata {...company}>
        <div>
            <Container h={{base:300}} size={""} w={"100vw"} m={0}  className='bg-no-repeat bg-cover relative flex   p-0 w-full' style={{backgroundPosition: "center", backgroundOrigin:"padding-box",backgroundSize:"100% 200%", backgroundImage:`url(${company.data?.data.imagePath})`}}>
                <Group style={{blur:"20px"}}  className='backdrop-blur-[100px]  bg-slate-50/20  gap-3 p-4  h-full w-full'>
                ...
                <Text className='text-wrap max-w-lg line-clamp-2'>{company.data?.data.description}</Text>
                <div className='grow'></div>
                <Text className='text-wrap max-w-lg text-3xl '>{company.data?.data.openingTime} /  {company.data?.data.closingTime}</Text>
            </div>
