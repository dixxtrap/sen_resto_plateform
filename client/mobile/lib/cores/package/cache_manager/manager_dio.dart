...
class DioCacheManager {
    ...
    if ((response.requestOptions.extra[DIO_CACHE_KEY_TRY_CACHE] ?? false) ==
      ...
      if (data != null) {
        try {
          ...
          if (baseResponse.sessionExpired == false &&
              baseResponse.status == true) {
            await _pushToCache(response);
          }
        } catch (e) {
          Logger.d("Can not cache this response");
        }
      }
    }
  ...
  String _getPrimaryKeyFromUri(Uri uri) {
    ...
    return "${uri.host}${uri.path}$param";
  }
  ...
  Future<bool> delete(String primaryKey,
    ...
    var key = "${_getRequestMethod(requestMethod)}-$primaryKey";
    return _manager.delete(key, subKey: subKey);
  }
